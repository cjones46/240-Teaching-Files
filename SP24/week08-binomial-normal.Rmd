...
Add a red partly transparent solid vertical line at the mean, 
dashed vertical lines one standard deviation above and below the mean, 
and dotted lines two standard deviations above and below the mean.

```{r}
gbinom(n, p, scale = TRUE) +
  geom_vline(xintercept = mu, color = "red", alpha = 0.5) +
  geom_vline(xintercept = mu + c(-1, 1) * sigma, linetype = "dashed") +
  geom_vline(xintercept = mu + c(-2, 2) * sigma, linetype = "dotted")
...
```{r}
ggplot() +
  geom_binom_density(n, p, a = 56, b = 70)
```
...
```{r}
left_tail = pnorm(75, mu, sigma)
right_tail = 1 - pnorm(125, mu, sigma)
...
* $Y = 24$ was $\frac{24-10}{7} = 2$ standard deviations away from its mean.
