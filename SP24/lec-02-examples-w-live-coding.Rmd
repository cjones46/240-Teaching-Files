...
* Logical operators take two objects, compare them, and return `TRUE` or `FALSE`.
...
* Finally, checking if a value is `NA` (R's stand-in for "Not Available", or "missing") is done with `is.na()`, NOT many people's first thought which is `== NA`.

```{r}
someMissingValues = c(1, NA, 2, NA, 3)

someMissingValues == NA # Incorrect; if you are in R Markdown, try hovering over the yellow caution sign to the left of this line!

is.na(someMissingValues)
```
