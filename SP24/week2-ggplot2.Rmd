...
* Notice that ggplot2 automatically chose a color scheme for us and created a legend.
    ...
    + While ggplot's default choices have been acceptable so far, the legend type it has chosen is perhaps not ideal here. We'll learn more about how to change this in Deeper Customization.
...
* Finally, we can and often do use both variable aesthetics and constant aesthetics in the same plot.
    ...
    + If you set, for example, `color = intervals` within `aes()` AND `color = "red"` within `geom_point`, the local `geom_point` "red" aesthetic would override, for the reason we are about to talk about! 
