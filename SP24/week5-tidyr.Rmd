...
* Whereas `full_join` keeps ALL rows, regardless of matching, `inner_join` keeps only those rows which match in both dataframes.
    - Once again, order does not matter in `inner_join`.
...
* Recall that in `full_join`, rows which completely matched had no missing data; and rows which matched only in one variable had some missing data.
...
* In this sense, `inner_join` can be replicated by doing a `full_join`, and then getting rid of any rows with `NA` values anywhere.
...
```{r}
#anti_join(band_instruments, band_members, join_by(name))
```
...
* **Write** a pivoting command which starts with `table2` and produces `table1`.
